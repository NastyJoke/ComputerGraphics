...
mat4 translation_matrix, total, frustum_matrix, camera_placement, rot, rot2, blade_rot, blade_time_rot, global_translation;
...
{    
	...
	camera_placement = lookAt(x, y, z,
							  mouseX, mouseY, 0,
							  0, 1, 0);

	global_translation = T(x, y, z);
	rot = Ry(t/1000);
	blade_time_rot = Rx(0);
...
void rotate_camera(int a, int b)
{
	mouseX = a;
	mouseY = b;
}
...
{
	x = 0; y = 5; z = -2; mouseX = 0; mouseY = 0;
	glutInit(&argc, argv);
	glutInitContextVersion(3, 2);
	glutCreateWindow ("GL3 white triangle example");
	glutDisplayFunc(display); 
	glutPassiveMotionFunc(rotate_camera);
	init ();
