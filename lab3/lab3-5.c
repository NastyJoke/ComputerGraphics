...
GLuint grass_tex, skybox_tex, conc_tex, earth_tex;
...
void init(void)
{
	...
	LoadTGATextureSimple("Earth.tga", &earth_tex);
	...
	glUniform1i(glGetUniformLocation(program, "texUnit1"), 2); // Texture unit 2
	
	glUseProgram(program);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, earth_tex);
	glUniform1i(glGetUniformLocation(program, "texUnit2"), 3); // Texture unit 3
...
void display(void) {
	...
	glUseProgram(program);
	glUniform3f(glGetUniformLocation(program, "xyz"), x, y, z); //camera position
	...
	// walls
	printError("walls");
	glUniform1i(glGetUniformLocation(program, "texUnit1"), 2);
	glUniform1i(glGetUniformLocation(program, "texUnit2"), 3);
