...
int texWidth;
static float speed = 0.2f; // camera speed
static float playerHeight = 1.0f;
...
void display(void)
{
	...
	camera.y = interpolate(speed, 1, camera, t_cam) + playerHeight;
	//position.y = calculateHeight(tm, position) +3 ;
	t_cam = camera.y;

	camMatrix = lookAt(camera.x, camera.y, camera.z, camera.x + directionX, camera.y, camera.z + directionZ, 0, 1, 0);
...
void timer(int i)
{
	...
	if (glutKeyIsDown('w')){ //move camera forward
		camera.x += directionX * 0.5f;
		camera.z += directionZ * 0.5f;
	}
...
{
	angle = 2.0f;
	camera.x = 0.0f; camera.z = 1.0f; camera.y = 3.0f;
