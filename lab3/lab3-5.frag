...
void main(void)
{
	...
	//out_Color = vec4(vshade, 1.0) * texture(texUnit1, exTexCoord) + vec4(vshade, 1.0) * texture(texUnit2, exTexCoord);
	out_Color = vec4(vshade, 1.0) * (texture(texUnit1, exTexCoord) * (0.5+sin(exSurface.z*15 + time)/5) + texture(texUnit2, exTexCoord) * (0.5-sin(exSurface.z*15 + time)/5) );
}
